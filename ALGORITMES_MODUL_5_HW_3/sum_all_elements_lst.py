...
def check_iter_data(data): #функция контроля принадлежности итерируемого эл_та к типу |int, float|
    ...
    return data
def sum_all_elements_lst(array: list) -> int or float:
    '''
    рекурсивная функция, вычисляющая сумму всех элементов массива
    :param array: [1,3,4,5]
    :return: 13
    ...
    len_array = len(array)
    if len_array > 2000: raise ValueError('В КАЧЕСТВЕ АРГУМЕНТА ПОЛУЧЕН СЛИШКОМ ДЛИННЫЙ СПИСОК ДЛЯ НАХОЖДЕНИЯ СУММЫ ЕГО ЭЛ_ОВ С ПОМОЩЬЮ РЕКУРСИИ !')
    ...
    return 0 if array == [] else check_iter_data(array[0]) + sum_all_elements_lst(array[1:])
